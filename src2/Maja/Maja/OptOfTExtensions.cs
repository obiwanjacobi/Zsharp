...
public static class OptOfTExtensions
{
    ...
    public static Opt<DateTime> TryParseDateTime(this string text)
        => DateTime.TryParse(text, CultureInfo.InvariantCulture, out DateTime val) ? new Opt<DateTime>(val) : Opt<DateTime>.Nothing;

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static Opt<DateTimeOffset> TryParseDateTimeOffset(this string text)
        => DateTimeOffset.TryParse(text, CultureInfo.InvariantCulture, out DateTimeOffset val) ? new Opt<DateTimeOffset>(val) : Opt<DateTimeOffset>.Nothing;
